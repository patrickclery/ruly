# WorkAxle QA Acceptance Testing Profile — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Create a `qa` profile that accepts acceptance criteria (from Jira tickets or PR descriptions), uses the existing `automation-test-qa` repo's Playwright infrastructure to write and run tests against dev.workaxle.com.
...
**Tech Stack:** Existing `automation-test-qa` Playwright + TypeScript infrastructure, Ruly profile (YAML), Ruly rules (Markdown), `gh` CLI, Jira CLI.
...
## Task 5: Add the QA Profile to profiles.yml

**Files:**
- Modify: `/Users/patrick/Projects/ruly/profiles.yml`
- Modify: `~/.config/ruly/profiles.yml` (sync)
- Modify: `/Users/patrick/Projects/chezmoi/config/ruly/profiles.yml` (sync)

**Step 1: Add the `qa` profile to `profiles.yml`**

Add after the `playwright` profile block (line ~434):
...
- `/Users/patrick/Projects/chezmoi/config/ruly/profiles.yml`
...
git commit -m "feat(qa): add qa profile to profiles.yml"
```
...
**Step 1: Verify profile squashes**
...
In a new Claude Code session with the QA profile loaded:
...
## Task 7: Wire QA Dispatch into Core Profile (Optional)

**Files:**
- Create: `rules/workaxle/core/standards/use-qa.md`
- Modify: `profiles.yml` — add to `core` profile
...
**Step 2: Add to core profile**

In `profiles.yml`, add to the `core` profile:
...
**Step 1: Add QA profile to profiles section**
...
cd /Users/patrick/Projects/ruly
git add README.md
git commit -m "docs: add QA profile and skills to README"
```
...
| 5    | Profile in profiles.yml                    | `profiles.yml` + synced copies                             |
| 6    | Test with PR 7495                        | Validation                                                |
| 7    | Wire into core (optional)                | `rules/workaxle/core/standards/use-qa.md` + `profiles.yml` |
