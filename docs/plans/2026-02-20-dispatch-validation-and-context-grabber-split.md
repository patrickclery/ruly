...
**Goal:** Prevent subagent profiles from including dispatch rules they can't execute, and split the monolithic `context-grabber` into source-specific subagents.

**Architecture:** Add `dispatches:` frontmatter to files that dispatch subagents. Validate during `ruly squash` that (1) standalone profiles register all dispatched types as subagents, and (2) subagent profiles never contain dispatch instructions. Split `context-grabber` into `context-jira`, `context-github`, `context-teams`, and `context-summarizer`. Move all `use-*.md` dispatch rules into `dispatches/` directories under their dispatcher domain.

**Tech Stack:** Ruby (ruly gem), Markdown rules, YAML profiles
...
Values are subagent names (the `name:` field from the parent profile's `subagents:` list). Collected transitively through `requires:` chains.

## Part 2: Validation

### Validation 1: Standalone profile dispatch check

After processing all sources, compare collected `dispatches:` against the profile's registered `subagents:`. Error if any dispatched type is not registered.

```
$ ruly squash core-reviewer

ERROR: Profile 'core-reviewer' dispatches: context_grabber
...
### Validation 2: Subagent profile dispatch check

During `process_subagents`, if any file in the subagent profile has `dispatches:`, error immediately.

```
$ ruly squash core

ERROR: Subagent 'core_reviewer' (profile: core-reviewer)
...
Order: Validation 1 runs first (main profile), Validation 2 runs during subagent generation.
...
## Part 4: Split context-grabber Into Source-Specific Profiles
...
Lives at `workaxle/core/dispatches/use-context-grabber.md`. Only loaded by dispatcher profiles.
...
## Part 6: Profile Impact

### Dispatcher profiles — replace context_grabber with 4 subagents
...
### Dispatcher profiles — update dispatch rule path

| Profile | Old path | New path |
|--------|----------|----------|
...
### Subagent profiles — remove dispatch rules, inline context fetching
...
Remove `use-context-grabber.md` from all subagent profiles.

### Dispatch rule files — move and add frontmatter

All `use-*.md` files move to `dispatches/` directories and get `dispatches:` frontmatter. All `profiles.yml` paths and `requires:` references updated.
