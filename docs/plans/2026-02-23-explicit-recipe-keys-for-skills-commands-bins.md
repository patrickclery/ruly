...
Bins are no longer auto-detected by path. Update the recipe to use `scripts:` key:
...
No recipes currently list bin files directly in `files:` — they come through directory expansion. The `scripts:` key handles this now.
