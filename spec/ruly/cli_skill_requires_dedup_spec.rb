...
RSpec.describe Ruly::CLI, type: :cli do
  ...
  describe 'skill requires deduplication against recipe' do
    ...
    context 'when a skill requires a file already in the recipe' do
      ...
      it 'does not inline the required file into the skill' do
        ...
        # Should NOT contain the accounts data since it's in the recipe
    ...
    context 'when a skill requires a file NOT in the recipe' do
      ...
      it 'does not inline into the skill but adds the required file to the recipe' do
        cli.invoke(:squash, ['test_recipe'])
        ...
        # requires: adds the file to the recipe (CLAUDE.local.md)
        recipe_content = File.read('CLAUDE.local.md', encoding: 'UTF-8')
        expect(recipe_content).to include('Alice')
        expect(recipe_content).to include('Team Directory')
      end
  ...
  describe 'duplicate skill requires warning' do
    ...
    context 'when squashing produces duplicate skill requires' do
      ...
      it 'adds the required file to the recipe (no duplication warning needed)' do
        output = capture(:stdout) { cli.invoke(:squash, ['test_recipe']) }
        # Requires are resolved into the recipe, so accounts.md appears once in CLAUDE.local.md
        # No duplication warning because the file is in the recipe
  ...
  describe 'skill requires deduplication in subagents' do
    ...
    context 'when subagent recipe includes a file also required by its skill' do
      ...
      it 'does not add subagent skill requires to the parent recipe' do
        ...
        recipe_content = File.read('CLAUDE.local.md', encoding: 'UTF-8')
        expect(recipe_content).not_to include('Alice')
      end
    end

    context 'when subagent skills have duplicate requires not in top-level recipe' do
      before do
