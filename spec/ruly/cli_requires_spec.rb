...
RSpec.describe 'Ruly::CLI requires feature' do
  ...
  describe '#resolve_requires_for_source' do
    ...
    it 'resolves requires from frontmatter' do
      ...
      allow(cli).to receive(:find_rule_file).with('rules/ruby/common.md').and_return("#{test_dir}/rules/ruby/common.md")
  ...
  describe 'integration: squash with requires' do
    before do
      ...
      # Set up gem root
      allow(cli).to receive(:gem_root).and_return(test_dir)
      allow(cli).to receive(:rules_dir).and_return("#{test_dir}/rules")
    end
    ...
    it 'includes required files in correct order' do
      ...
      local_sources, _, _ = cli.send(:process_sources_for_squash, sources, 'claude', {}, options)
      ...
      expect(paths.any? { |p| p == 'rules/testing/integration.md' }).to be true
    ...
    it 'deduplicates required files' do
      ...
      expect(paths.size).to eq(3)
      expect(paths.any? { |p| p.end_with?('rules/ruby/common.md') }).to be true
      expect(paths.any? { |p| p == 'rules/ruby/rspec.md' }).to be true
      expect(paths.any? { |p| p == 'rules/ruby/sequel.md' }).to be true
    end
