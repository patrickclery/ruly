...
See [PR common patterns](../common.md) for the MCP-first approach principles and when to use CLI fallbacks.

## Initial Setup

### 1. Detect Repository Context

See [Repository context detection](../common.md#repository-context-detection) for automatic repository detection patterns.
...
.ruly/bin/common/countdown-timer.sh 30 "Waiting for reviewers to process"
...
For manual resolution if needed, see [PR comment resolution](../pr-comment-resolution.md).
...
# Display active countdown timer using the extracted script
.ruly/bin/common/countdown-timer.sh $WAIT_TIME "Waiting before next check"
