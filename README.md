...
├── .ruly/
│   └── bin/             # Executable scripts (from bins: key)
└── .claude/
...
2. **Organize your rules:**
   ...
   **Directory conventions:**
   ...
   - `bin/` - Shell scripts and executables. Must be listed under the `bins:` recipe key to be copied to `.ruly/bin/`.
...
recipes:
  my-recipe:
    ...
    bins:
      # Copied to .ruly/bin/ as executables
...
| `bins:` | `.ruly/bin/` | Copies scripts as executables |
...
- `bins:` directories expand to all `*.sh` files
